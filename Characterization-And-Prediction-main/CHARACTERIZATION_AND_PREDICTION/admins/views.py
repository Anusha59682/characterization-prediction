...
def charts(request, chart_type):
    ...
    if chart_type == 'all':
        ...
        d = Feedback.objects.filter(product__product_name='laptop').values('product__vendor_name').annotate(dcount=Count('rating'))
    elif chart_type == 'mobileaccessories':
        d = Feedback.objects.filter(product__product_name='mobile accessories').values('product__vendor_name').annotate(dcount=Count('rating'))
    elif chart_type == 'watches':
        d = Feedback.objects.filter(product__product_name='watches').values('product__vendor_name').annotate(dcount=Count('rating'))
    elif chart_type == 'shoes':
        d = Feedback.objects.filter(product__product_name='shoes').values('product__vendor_name').annotate(dcount=Count('rating'))
...
def delete_product(request, id):
    product = get_object_or_404(Prodcuts, id=id)
    product.delete()
    return redirect('admins:home')
